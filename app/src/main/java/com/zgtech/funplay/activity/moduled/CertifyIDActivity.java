...
public class CertifyIDActivity extends BaseActivity {
    @Bind(R.id.ll_back)
    LinearLayout llBack;
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_push:
                ...
                if (TextUtils.isEmpty(positiveUrl)) {
                    ...
                    doCertify(name, idNumber);
                }
                break;
        }
    }

    private void doCertify(String name, String idNumber) {
        HashMap map = new HashMap();
        map.put("identType", "0");
        map.put("identTitle", name);
        map.put("identRemark", idNumber);
    ...
    private void monitorGalleryPick() {
        iHandlerCallBack = new IHandlerCallBack() {
            ...
            public void onSuccess(List<String> photoList) {
                ...
                dialog = new ProgressDialog(CertifyIDActivity.this);
...
//                .multiSelect(true, maxSize)                   // 配置是否多选的同时 配置多选数量   默认：false ， 9
    ...
    private void initPermissions() {
        if (ContextCompat.checkSelfPermission(CertifyIDActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            Log.i(TAG, "需要授权 ");
            if (ActivityCompat.shouldShowRequestPermissionRationale(CertifyIDActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                Log.i(TAG, "拒绝过了");
                Toast.makeText(CertifyIDActivity.this, "请在 设置-应用管理 中开启此应用的储存授权。", Toast.LENGTH_SHORT).show();
            } else {
                Log.i(TAG, "进行授权");
                ActivityCompat.requestPermissions(CertifyIDActivity.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, REQUEST_CODE);
            }
        } else {
            Log.i(TAG, "不需要授权 ");
            GalleryPick.getInstance().setGalleryConfig(galleryConfig).open(CertifyIDActivity.this);
        }
    }
    ...
    private void readyUpImgsToServer(ArrayList<String> originPhotoList) {
        ...
        Luban.compress(CertifyIDActivity.this, fileList)
            ...
            private void ifSuccess(Response<UpImgsModel> response) {
                if (response.body().getCode() == 2) {
                    ...
                    if (isPositive) {
                        ...
                        Glide.with(CertifyIDActivity.this)
